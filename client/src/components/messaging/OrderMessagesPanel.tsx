...
export function OrderMessagesPanel({ orderId, orderNumber }: OrderMessagesPanelProps) {
    ...
    return (
        <Card className="border-stone-200 shadow-warm overflow-hidden">
            <div className="bg-stone-50 border-b border-stone-200 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <MessageCircle className="w-5 h-5 text-accent" />
                    <h3 className="text-lg font-display font-semibold">Assistenza Ordine</h3>
                </div>
                <span className="text-xs text-stone-600 font-medium tracking-tight">#{orderNumber}</span>
            </div>
                    ...
                    {isLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center py-10">
                            ...
                            <p className="text-sm text-stone-600 font-medium max-w-[200px]">
                                Nessun messaggio trovato. Hai domande sul tuo ordine? Scrivici qui sotto.
                            </p>
                        </div>
                    ) : (
                                    ...
                                    <div className="flex items-center gap-2 mb-1">
                                        {msg.isAdminMessage ? (
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className="text-[10px] uppercase tracking-wider font-bold text-accent">Supporto</span>
                                            </div>
                                        ) : (
                                            <span className="text-[10px] uppercase tracking-wider font-bold text-stone-600">Tu</span>
                                        )}
                ...
                <div className="p-4 border-t border-stone-200 bg-stone-50/50">
                    <div className="flex flex-col gap-3">
                        <Textarea
                            ...
                            rows={3}
                            className="resize-none border-stone-300 focus-visible:ring-accent/20 bg-white text-stone-900 placeholder:text-stone-500"
                        />
                        <Button
                            onClick={handleSend}
                            disabled={!newMessage.trim() || sendMessageMutation.isPending}
                            className="w-full bg-accent hover:bg-accent/90 text-white rounded-xl shadow-warm border-none h-11"
                        >
                        ...
                        <p className="text-[10px] text-center text-stone-600 font-medium">
                            Riceverai una notifica via email quando riceverai una risposta.
                        </p>
                    </div>
                </div>
            </CardContent>
